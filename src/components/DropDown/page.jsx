  ...
  return (
    ...
    <ul className={'header_center_list'}>
      {data?.map((element) => (
          ...
          <Menu.Items className={'nav_menu_list'}>
            ...
            {element?.children?.map((childElement) => (
              <Menu.Item key={childElement?.id}>
                ...
                ) : <Link className={'header_menu_link'} id='RouterNavLink' href="/account-settings">{childElement?.title}</Link>}
        ...
        </Menu></li>) : (<Link className={'header_menu_element_link'} key={element?.id} href={element?.path}>{element?.title}</Link>))
